...
class DropBotDxAccessoriesPlugin(Plugin, AppDataController, StepOptionsController):
    ...
    def connect(self):
        ...
        # reference
        try:
            service = get_service_instance_by_name('microdrop.dropbot_dx')
            if service.enabled():
        ...
        try:
            service = get_service_instance_by_name('microdrop'
    ...
    def get_step_label(self):
        try:
            step_label_plugin =\
                get_service_instance_by_name('microdrop.step_label_plugin')
    ...
    def get_schedule_requests(self, function_name):
        ...
        if function_name in ['on_plugin_enable']:
            return [ScheduleRequest('microdrop.dropbot_dx', self.name),
                    ScheduleRequest('microdrop.dmf_control_board_plugin',
                                    self.name)]
        elif function_name == 'on_step_run':
            return [ScheduleRequest('microdrop.dmf_device_ui_plugin',
                                    self.name)]
